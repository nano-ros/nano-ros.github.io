...
The hardware_interface is a C abstract interface. ros2_HDL generates a concrete implementation. Application code interacts only with this abstraction.
...
When `base_addr` is NULL, the implementation generated by ros2_HDL reads and writes to an in-memory buffer instead of hardware registers. This simulation backend is used for all CI tests and Space ROS integration tests that do not require real hardware.